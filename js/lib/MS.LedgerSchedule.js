...
MS.LedgerSchedule = function(config){
    ...
    this.generateDateItems = function(record){	
		...
		}else if(record.data.repeat = 'Monthly'){
			if (record.data.start_date) {
				console.log("record", record.data)
				record.data.start_date
				if (record.data.start_date.getDate) {
					recDay.setDate(record.data.start_date.getDate());
				} else {
					var cdate = Date.parseDate(record.data.start_date, "m/d/Y");
					recDay.setDate(cdate.getDate());
				}
			} else {
